...
for shop in (freelancers,antiques,pet_shop) :
    ...
    buy_item = input(f'Chào mừng đến {shop["name"]}! bạn muốn mua gì: {shop}').lower()
    # Cập nhật giỏ hàng
    cart.update({buy_item:shop.pop(buy_item)}) # sử dụng pop...
    buy_items = ", ".join(list(cart.keys()))
print(f'Bạn đã mua {buy_items}. Hôm nay tất cả đều miễn phí. Chúc bạn có một ngày hỗn loạn vui vẻ!')